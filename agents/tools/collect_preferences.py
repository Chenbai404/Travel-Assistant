...
def collect_preferences(params: PreferenceInput) -> PreferenceExtractionResult:
    ...
    Args:
        params: Contains user_input and optional current_preferences
...
"""
    ...
    # Build user message
    user_message = f"Extract travel preferences from this input: {params.user_input}"
    if params.current_preferences:
        user_message += f"\n\nCurrent preferences already collected: {json.dumps(params.current_preferences, indent=2)}"
    ...
    try:
        ...
        # Merge with current preferences if provided
        if params.current_preferences:
            for key, value in params.current_preferences.items():
